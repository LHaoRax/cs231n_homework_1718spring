...
def relu_forward(x):
    ...
    ###########################################################################
    out = np.maximum(x, 0)  # use the maximum method for keep the shape for x
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = x
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    pass
    x, gamma, beta, x_hat, xmu, var, eps = cache
    N, D = dout.shape
    ...
    # step5
    dvar = 0.5 * dsqrtvar * 1 / np.sqrt(var + eps)
    ...
    # step2
    dmean = -1 * np.sum(dxmiu1 + dxmiu2, axis=0)
    dx1 = dxmiu1 + dxmiu2
...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean = np.sum(-dx_hat / np.sqrt(sample_var + eps), axis=0) + dvar * -2 * np.sum(x - sample_mean, axis=0) / m

    dgamma = np.sum(dout * x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)

    dx = dx_hat * 1 / np.sqrt(sample_var + eps) + dvar * 2 * (x - sample_mean) / m + dmean / m
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out = gamma * x_hat.transpose((1, 0)) + beta
    cache = (x, gamma, beta, x_hat, sample_mean, sample_var, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    # step8
    dgamma = np.sum(dgammax * x_hat.transpose(1, 0), axis=0)
    dxhat = dgammax * gamma  # (N, D)
    dxhat = dxhat.transpose(1, 0)  # (D, N)
    # step7
    dxmiu1 = dxhat * ivar
    divar = np.sum(dxhat * xmu, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(out_H):
        for j in range(out_W):
            ...
            for k in range(F):
                out[:, k, i, j] = np.sum(w[k, :, :, :] * x_padded_mask, axis=(1, 2, 3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_H):
        for j in range(out_W):
            ...
            dx_mask += flag * (dout[:, :, i, j])[:, :, None, None]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    for i in range(number):
        x_group = x[:, i * G: (i + 1) * G, :, :]
        gamma_group = gamma[:, i * G: (i + 1) * G, :, :]
        beta_group = beta[:, i * G: (i + 1) * G, :, :]
        x_group = x_group.transpose(0, 2, 3, 1).reshape(N * H * W, G)
        temp, cache = layernorm_forward(x_group, gamma_group, beta_group, gn_param)
        out[:, i * G: (i + 1) * G] = temp
    out = out.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
