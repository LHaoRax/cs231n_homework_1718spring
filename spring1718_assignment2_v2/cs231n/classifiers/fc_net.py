...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        pass
        # first hidden layer and dropout
        """
        
        
        hidden_layers = list(range(self.num_layers + 1))
        cache_list = list(range(self.num_layers))
        hidden_layers[0] = X

        for i in range(self.num_layers):
            W, b = self.params['W' + str(i + 1)], self.params['b' + str(i + 1)]
            # 最后一层不需要gamma 和 beta参数
            if i == self.num_layers - 1:
                hidden_layers[i + 1], cache_list[i] = affine_forward(hidden_layers[i], W, b)
            else:
                if self.normalization == 'batchnorm':
                    gamma, beta = self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)]
                    hidden_layers[i + 1], cache_list[i] = affine_bn_relu_forward(hidden_layers[i], W, b,
                                                                                 gamma, beta, self.bn_params[i])
                else:
                    hidden_layers[i + 1], cache_list[i] = affine_relu_forward(hidden_layers[i], W, b)
        ...
        loss, dout = softmax_loss(scores, y)
        dhidden_layers = list(range(self.num_layers + 1))
        dhidden_layers[-1] = dout
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dhidden_layers[i - 1], grads['W' + str(i)], grads['b' + str(i)] \
                    = affine_backward(dhidden_layers[i], caches[i - 1])
            else:
                if self.normalization == 'batchnorm':
                    dhidden_layers[i - 1], grads['W' + str(i)], grads['b' + str(i)], \
                    grads['gamma' + str(i)], grads['beta' + str(i)] \
                        = affine_bn_relu_backward(dhidden_layers[i], caches[i - 1])
                else:
                    dhidden_layers[i - 1], grads['W' + str(i)], grads['b' + str(i)] = \
                        affine_relu_backward(dhidden_layers[i], caches[i - 1])
